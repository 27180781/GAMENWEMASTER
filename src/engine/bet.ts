 ...
 *
 * החוזה מול מערכת יצירת המשחקים (מה בדיוק היא שולחת בשקופית `bet`, ואיך
 * `majorityDecides` ו-`liveVoteCounts` יוצאים ממנה) מתועד אצלה ב-ENGINE-bet.md,
 * ו-tests/builderContract.test.ts מזין את הפלט הזה כלשונו דרך הטוען האמיתי.
 */
