...
בהן לא השתנה). שורת `_pre` היא כול-או-כלום: בלי הפתיח גם המספר/השם שאחריו נופל,
אחרת נשמע מספר ערום.
...
| כניסה לשאלה שאינה הראשונה | `amb_next_*` **לפני** מספר השאלה, באותו משפט; בשאלה שחוצה את החצי (ורק ממשחק של 6 שאלות ומעלה, פעם אחת) `amb_half`; בשאלה האחרונה `amb_last_question` — כל אחד מהם **במקום** `amb_next_*` |
| חלף חצי מזמן ההצבעה | `amb_voting_*` (פעם אחת לחלון) |
| ~3 שניות אחרונות | `amb_hurry`; כשהוא נופל על אותו צעד עם `timer_ten_left` — «עשר שניות» מנצח ו-`amb_hurry` נופל |
| חשיפת התשובה הנכונה | אחרי המשפט העובדתי: כולם צדקו `amb_all_correct_*` · אף אחד `amb_none_correct_*` · יותר משני שלישים `amb_most_correct` · פחות מחמישית `amb_few_correct`. שקט כשהמספרים לא ידועים, בסקר, בהימור וב«הרוב קובע» |
| לוח מובילים | אחרי הפודיום: `amb_lead_change` (המוביל התחלף מאז ההכרזה הקודמת) · `amb_tie_top` (תיקו בראש) · אחרת `amb_lead_gap_pre` + ההפרש + `unit_points` |
| מסך הקבוצות | `amb_group_close` כשההפרש קטן מעשירית · אחרת `amb_group_lead_pre` + קטע שם הקבוצה · ובלי קטע שם `amb_group_gap_pre` + ההפרש |
