...
- `groups` — מילון `שם הקבוצה בדיוק כמו במרשם → url` לקטעי שמות הקבוצות (הם משתנים
  ממשחק למשחק ולכן אינם בבנק). שם חסר = הקריין אומר את הביטוי הכללי בלי השם, וכך גם
  שם שהקטע שלו לא נטען (`playableGroupClips` — הקטעים נטענים מראש יחד עם הבנק).
...
- **הקטעים אינם מדיית משחק.** הם מסומנים `kind: 'narration'` ב-`mediaFields`, ולכן
  ...
  `zipLoader`, למיפוי הנתיבים היחסיים (בנק, `groups` וקטעי השקופיות); קטע שחסר ב-ZIP
  אינו «נכס חסר» אלא מדולג בשקט. `NarrationPlayer.preload` מושך את הבנק ואת השקופית
  הבאה ברקע, בלי לחסום. הנגן מושך ב-`fetch()`, ולכן ב-EXE סכימת `trivia-media://`
  רשומה `corsEnabled` וכל תשובה שלה נושאת `Access-Control-Allow-Origin` (`mediaScheme.cjs`).
- **עריכה חיה אינה משקרת.** המנוע לא מייצר שמע, ולכן `slideEdit` שומר כל קטע צמוד לטקסט
  שהוקלט (`keepNarrationTruthful`): עריכת נוסח מורידה את הקטע שלו, תשובה שנמחקה או זזה
  לוקחת את הקטע שלה איתה, «התשובה הנכונה» יורד כשהתשובות הנכונות משתנות, ושקופית חדשה
  נוצרת בלי קריינות. שקט עדיף על קטע שגוי.
...
**נעילת autoplay (דפדפן/טלפונים).** עד האינטראקציה הראשונה של המפעיל הנגן אינו משמיע
דבר. `NarrationPlayer.isUnlocked` (אינטראקציה, sticky activation, או הקשר שרץ בלעדיה
כמו ב-EXE) נכנס לבמאי כ-`audioUnlocked`: עד אז אירוע של «פעם אחת למשחק»
(`flow_welcome`, `amb_start_*`, `lb_title` + `flow_thanks` בלוח הניקוד) **אינו נאמר ואינו
נרשם**, ו-`amb_half` עובר לשאלה הבאה; ה-host מריץ צעד ברגע הפתיחה, כך שמה שעדיין
מתאים למסך נאמר אז. משפטי השקופית נצרכים כרגיל ואינם חוזרים באיחור. משפט שמגיע אחרי
האינטראקציה אבל לפני שה-resume הסתיים (המקש שפתח את האודיו החליף גם את המסך) ממתין לו.

- **מה נחשב פתיחה:** `click` או `keydown` (מאזינים ב-capture) ש-`navigator.userActivation`
  מכיר כאינטראקציה — Esc ומקש שינוי לבדו אינם. לא `pointerdown`: הוא מקדים בעשירית
  שנייה את הקליק שמחליף את המסך. הדגל נדלק בתוך האירוע וה-host קורא אותו בצעד, כך
  שהמסך שהאירוע פותח כבר נחשב פתוח; ההודעה (`onUnlock`) יוצאת במשימה הבאה.
- **«ברוכים הבאים»** ממתין ללובי עצמו: מעל שכבה (תפריט, הגדרות, מרשם) הוא אינו נאמר.
- **«מתחילים!» שנדחה** תקף רק לביקור שבו המשחק התחיל. שקופית אחרת או שכבה מוקראת
  מעליו מורידות אותו בשקט (נרשם כנאמר); תפריט המפעיל רק משהה אותו. דחייה בגלל מדיה
  חוסמת נשארת עד שהקריאה נאמרת, כמו קודם.

מעבר אוטומטי (`autoTransition`) **ממתין לסיום המשפט** כשהקריין מדבר, כדי לא לחתוך אותו
באמצע, וכך גם הסגירה האוטומטית של מסך תוצאות ההימור; לחיצת מנחה עדיין מקדמת מיד. בתפריט המפעיל (ESC) יש מתג «קריינות» וווליום נפרד
