...
describe('questionLabel — רשימות, תפריט המפעיל ודוח התוצאות', () => {
  ...
  it('★ שאלת תמונה עם טקסט ב-que — הטקסט משמש כשם פנימי', () => {
    // המערכת שולחת que ריק, אבל אם תשלח שם — עדיף להציג אותו בדוח ובחיפוש
    // מאשר תווית גנרית זהה לכל השאלות.
    expect(
      questionLabel(q({ que: 'שאלת הלוגו', queMode: 'image', src: 'https://x/a.jpg' })),
    ).toBe('שאלת הלוגו');
  });
